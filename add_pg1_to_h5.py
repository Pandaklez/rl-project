...
add_pg1_to_h5.py
─────────────────
Append PG1 SMPLer-X results for missing subjects into an existing H5.
Opens the H5 in r+ mode and writes PG1 subgroups alongside existing data.

Usage:
    python add_pg1_to_h5.py \
        --h5       lifted_movi_part1_upd2.h5 \
        --results  demo/results \
        --v3d_dir  F_Subjects_meta \
        --subjects 10 26 45 49 58
"""
...
def main():
    ...
    parser.add_argument("--h5",       required=True, help="Path to existing H5")
    ...
    with h5py.File(args.h5, "r+") as h5f:
        for subj_id in args.subjects:
            pg1_dir = results_root / f"F_PG1_Subject_{subj_id}_L" / "smplx"
            if not pg1_dir.exists():
                log.warning(f"Subject {subj_id}: no smplx dir at {pg1_dir}, skipping")
                skipped += 1
                continue

            frame_map = load_frame_map(pg1_dir)
            if not frame_map:
            ...
            for action, (start, end) in zip(action_names, action_inds):
                ...
                if "PG1" in clip_grp:
                    log.debug(f"  {clip_name}/PG1 already exists, skipping")
                    skipped += 1
                    continue
                ...
                written += 1
                log.info(f"  wrote {clip_name}/PG1  T={poses.shape[0]}")

    log.info(f"Done. Wrote {written} PG1 clips, skipped {skipped}.")

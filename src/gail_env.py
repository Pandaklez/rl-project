...
class GymMoviEnv(gymnasium.Env):
    ...
    * `reward_mode="reproj"` — reprojection + smoothness, using no ground truth
    ...
    Experiment (C) adds a third term on top of either mode: `disc_reward`, a
    `GAILRewardProvider` (`src/models/discriminator.py`) wrapping a
    discriminator trained concurrently in `src/gail_train.py`. Unlike
    `reproj_reward` this term is not a fixed function of the data — its
    weights change every rollout — but the env's job is unchanged: hold a
    reference and call `.score()`. See `GAILRewardProvider` for why that is
    enough for the reward to track training without the env itself changing.
    """
    ...
    def __init__(
        ...
        # Experiment (C): additive GAIL-style term, on top of whichever base
        # reward_mode is in force. `disc_reward is None` reproduces experiments
        # (A)/(B) exactly — `w_gail` alone is not enough to disable it because
        # 0.0 is also a legitimate ablation value to sweep.
